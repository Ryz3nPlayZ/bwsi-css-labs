...
def test_fractional_inputs():
    assert simple_calculator("add", 2.5, 0.5) == 3.0
    assert simple_calculator("multiply", 1.5, 2) == 3.0
    assert simple_calculator("subtract", -1.5, 2.5) == -4.0


def test_large_and_negative_values():
    assert simple_calculator("add", 1_000_000, 2_000_000) == 3_000_000
    assert simple_calculator("multiply", -100, -5) == 500
...
def test_invalid_operation():
    ...
    with pytest.raises(ValueError, match="Invalid operation. Please choose from 'add', 'subtract', 'multiply', or 'divide'."):
        simple_calculator("ADD", 5, 3)                  # Test case-sensitive invalid operation
